...
def main():
    # Loading the Dataset
    df = pd.read_csv("E:\VS Code Workspaces\Projects\C++ Projects\Pattern Recognition\SportsDomain.csv")
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
    ...
    X[["Average points scored"]] = MinMaxScaler().fit_transform(X[["Average points scored"]])
    categorical = categorical.apply(le.fit_transform)
    y = le.fit_transform(y)
    X = pd.merge(X[["Average points scored"]], categorical, left_index=True, right_index=True)
    X = X.values
    ...
    plt.title("Table Tennis Wins Predictor")
    plt.show()
